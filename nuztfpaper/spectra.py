...
from nuztfpaper.style import (base_height, base_width, big_fontsize, data_dir,
                              dpi, output_folder, plot_dir)
...
def load_spectrum(path: str, smooth: int = 1):
    if ".fits" in path:
        ...
        if "wl" not in data.columns:
            if "loglam" in data.columns:
                data.insert(1, "wl", 10.0 ** data["loglam"])
...
def plot_spectrum(
    ...
    if host_spectrum is not None:
        ...
        mask = np.logical_and(host["flux"] > 0.0, host["flux"] != np.nan)
        host["flux"][~mask] = 0.00
        ...
        plt.plot(
            host["wl"] / (host_redshift + 1.0),
            host["flux"] / hscale + y_offset,
            color="C2",
            alpha=0.5,
        )
        ...
        plt.plot(
            host_smoothed["wl"] / (source_redshift + 1.0),
    ...
    plt.plot(
        data_smoothed["wl"] / (source_redshift + 1.0),
        data_smoothed["flux"] / scale + 1.0,
        color="C7",
        label=f"{source_label} (smoothed)",
    )
    ...
    if comparison_spectrum is not None:
        ...
        plt.plot(comp["wl"] / (comp_redshift + 1.0), y, color="C3", label=comp_label)
    ...
    rslim = ax1.get_xlim()
    ax1b.set_xlim(
        (rslim[0] * (1.0 + source_redshift), rslim[1] * (1.0 + source_redshift))
    )
    ax1.set_xlabel(r"Rest Wavelength [$\rm \AA$]", fontsize=big_fontsize)
    ax1b.set_xlabel(
        rf"Observed Wavelength (z={source_redshift:.3f})", fontsize=big_fontsize
    )
